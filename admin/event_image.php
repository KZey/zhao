...
if ((($filetype == "image/gif") || ($filetype == "image/jpeg") || ($filetype == "image/png")) && ($filesize < 20000))
{
  if ($_FILES["file"]["error"] > 0)
    ...
    $sql = "insert into event_image values('', ".$publicRelationId.", '".$picture."', ".$status.") ";
    echo $sql."<br/>";
    ...
    if ( $result ) {
           ...
           $_SESSION['iflogin']= 1;
           header("Location: event_image.php?id=".$publicRelationId);
        } else {
          echo  $err[] = "Operation failed, please check what you input or contact system admin, <a href='event_image.php?id=".$publicRelationId.">Go back</a> ";
        }
...
<a href='event_edit.php?id=<?php echo $_GET['id']; ?>'>Back</a>
                <form class="form-horizontal" action='event_image.php' method='post' enctype="multipart/form-data">
...
<?php 
    $id = $_GET['id'];
    $dataset=mysql_query("select * from event_image where eventId = ".$id." order by id desc");
...
?>
                                            ...
                                            <td class="center"><a href='event_image_delete.php?id=<?php echo $result['id'];?>&event_id=<?php echo $id;?>'>Delete</a></td>
